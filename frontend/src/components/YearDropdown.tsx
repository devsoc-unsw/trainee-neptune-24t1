...
const YearDropdown: React.FC<DropdownProps> = ({ years, toggleDropdown, year, setYear, setDropdown, dropdown, dropdownRef}) => {
  ...
  return (
    <div className="relative flex-col mt-2 dark:text-white" ref={dropdownRef}>
      <button className="flex text-2xl font-bold items-center font-sans rounded px-3 py-1 transition duration-300 hover:bg-gray-100 dark:hover:bg-zinc-800" onClick={toggleDropdown}>
        <span>{year}</span>
        <FontAwesomeIcon className="pl-2" icon={faCaretSquareDown} size="xs"/>
      </button>
      {dropdown && (
        <div className="absolute w-full flex flex-col bg-white dark:bg-zinc-900 border border-gray-300 rounded shadow overflow-y-auto">
          {years.map((year) => (
